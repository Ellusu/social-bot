<div id="messengerbot_meta_item">
        ...
        if ( count( $messengerbotDetails ) > 0 && is_array($messengerbotDetails)) {
            foreach( $messengerbotDetails as $messengerbotDetail ) {
                ...
                if ( isset( $messengerbotDetail['command'] ) || isset( $messengerbotDetail['response'] )  || isset( $messengerbotDetail['response_type'] ) ) {
                    ...
                    <span class="categoryBlock">'.__('Category').':</span>'.scb_getOptionCategory(true, $response_cat).'
                    <span class="tagBlock">'.__('Tag').':</span>'.scb_getOptionTag(true, $response_tag).'
                    <p></p> <a href="#" class="remove-package">%5$s</a></div>', $c, $messengerbotDetail['command'], $messengerbotDetail['response_type'], $messengerbotDetail['response'], 'Remove' );
                    $c = $c +1;
                }
            }
        }
    ...
    <a href="#" class="add_package button add_command"><?php __('Add Messenger Bot'); ?></a>
    <script>
