<?php
    ...
    function scb_add_command_script() {
        ...
        $command = sanitize_text_field( $_POST['command'] );
            ...
            Command:<input type="text" name="messengerbotDetails['.$c.'][command]" value="'.$command.'" />
            Type:'.scb_getOptionType(false, false, esc_attr($c)).'<span class="textBlock">
            Response:</span> <textarea name="messengerbotDetails['.$c.'][response]" rows="4" cols="50"  class="textBlock" ></textarea>
            <span class="categoryBlock">Categoty:</span>'.scb_getOptionCategory(false, false, esc_attr($c)).'
            <span class="tagBlock">Tag:</span>'.scb_getOptionTag(false, false, esc_attr($c)).'
